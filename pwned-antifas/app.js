...
var $author$project$Main$init = function (hashes) {
	return _Utils_Tuple2(
		{au: false, av: '', aw: hashes},
...
var $author$project$Update$isListed = F2(
	function (model, value) {
		var cropped = A3($elm$core$String$slice, 0, 100, value);
		return _Utils_update(
			model,
			{
				au: A2($elm$core$List$member, cropped, model.aw),
				av: cropped
			});
	});
var $author$project$Update$update = F2(
	function (msg, model) {
		var value = msg;
		return _Utils_Tuple2(
			A2($author$project$Update$isListed, model, value),
			$elm$core$Platform$Cmd$none);
	});
var $author$project$Update$UpdateValue = $elm$core$Basics$identity;
...
var $author$project$View$view = function (model) {
	var status = ($elm$core$String$length(model.av) < 100) ? {G: 'alert alert-secondary', B: 'Informe os 100 caracteres do hash do seu nome (pode colar tudo gue a gente descarta o que sobrar).', D: ''} : (model.au ? {G: 'alert alert-danger', B: 'Encontramos no dossiê hashes com os mesmos caracteres que você informou.', D: 'Cuidado'} : {G: 'alert alert-success', B: 'Não existem hashes que iniciam com os mesmos caracteres do hash que você digitou.', D: 'Boa notícia'});
	var message = $elm$core$String$isEmpty(status.D) ? _List_fromArray(
		[
			$elm$html$Html$text(status.B)
		]) : _List_fromArray(
		[
			A2(
			$elm$html$Html$h3,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class(status.G)
					]),
				message)
			]));
};
